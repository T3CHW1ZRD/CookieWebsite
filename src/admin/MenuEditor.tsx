  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="font-display text-2xl">Flavours</h2>
          <p className="text-xs text-muted-foreground mt-1">
            These appear as the typographic flavour list on the site. Photos live in
            the <strong>Gallery</strong> tab.
          </p>
        </div>
        <button onClick={add} className="btn-primary text-sm py-2 px-4 shrink-0">
      ...
      {content.menu.length === 0 && (
        <p className="rounded-2xl border border-dashed border-border bg-background p-10 text-center text-muted-foreground">
          No flavours yet. Click <strong>Add item</strong> to create your first one.
        </p>
      )}
